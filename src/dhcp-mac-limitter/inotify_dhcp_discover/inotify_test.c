/* Modified by Shintaro Fujiwara
 * shintaro.fujiwara@gmail.com
 * This program was originaly written by IBM for educational purposes.
  ...
  if (inotify_fd > 0)
    {
	  ...
	  //wd = watch_dir (inotify_fd, argv[index], IN_ALL_EVENTS & IN_MODIFY );
	  wd = watch_dir (inotify_fd, "/tmp/dhcp-discover.log", IN_ALL_EVENTS & IN_MODIFY );
      //	}
